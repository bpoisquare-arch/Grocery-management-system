...
export function AddGroceryModal({ open, onOpenChange }: AddGroceryModalProps) {
  ...
  const totalSpent = groceryEntries
    .filter((entry) => entry.entity === activeEntity)
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files[0]) {
      ...
      if (validTypes.includes(file.type)) {
        setSlipFile(file);
        toast.success(`Slip file "${file.name}" ready to upload.`);
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const status: SlipStatus = slipFile ? "Slip Uploaded" : "Slip Missing";
      ...
      toast.success(`Grocery expense of Rs. ${newAmount.toLocaleString()} added successfully.`);
      if (slipFile) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-3xl w-full bg-white p-6 md:p-8 rounded-xl ring-1 ring-black/5 shadow-xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-xl font-bold text-gray-900">Add Grocery</DialogTitle>
          <DialogDescription className="text-sm text-gray-500">
            Add a new grocery expense for {activeEntity} Entity.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-12 gap-6 mt-4">
          ...
          <div className="md:col-span-7 space-y-4">
            ...
            {/* Slip Upload Area */}
            <div className="flex flex-col gap-1.5">
              <Label className="text-xs font-semibold text-gray-700">SLIP UPLOAD (OPTIONAL)</Label>
              <div
                ...
                className={cn(
                  ...
                  slipFile ? "border-emerald-500/50 bg-emerald-50/10" : ""
                )}
              >
                <input
                  type="file"
                ...
                <label htmlFor="file-upload" className="cursor-pointer flex flex-col items-center gap-1.5">
                  ...
                  <span className="text-xs font-semibold text-gray-900">
                    {slipFile ? slipFile.name : "Upload Grocery Slip"}
                  </span>
                  <span className="text-[10px] text-gray-500">
                    {slipFile ? "Click or drag to replace" : "Drag and drop or browse files (JPG, PNG, PDF)"}
                  </span>
                </label>
              </div>
            </div>
            ...
            <Card className="border-gray-200 bg-slate-50/50 shadow-none">
              <CardContent className="p-4 space-y-4">
                ...
                <div className="space-y-2 text-xs font-medium text-gray-600">
                  ...
                  <div className="flex justify-between text-gray-900 border-t border-gray-200/60 pt-2">
                    <span>New Grocery Amount</span>
                ...
                {isOverBudget && (
                  <Alert className="bg-red-50 border-red-100 text-red-800 p-2.5 rounded-lg flex items-start gap-2 shadow-2xs">
                    ...
                    <AlertDescription className="text-[11px] leading-snug">
                      <strong>⚠ Over Budget:</strong> This expense exceeds the monthly allocated budget.
                    </AlertDescription>
                  </Alert>
                )}
              </CardContent>
