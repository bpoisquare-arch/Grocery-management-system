...
export function AddCommissionModal({ open, onOpenChange }: AddCommissionModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIsLoading(false);
      onOpenChange(false);
      toast.success(`Commission entry for ${studentName} added successfully.`);

      // Reset form
      setStudentName("");
      setService("Visa Processing");
      setCounselor(counselors[0]?.name || "Humaira Amin");
      setAmountStr("");
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[580px] max-h-[90vh] overflow-y-auto bg-white p-6 rounded-2xl shadow-xl border border-gray-100">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <DialogDescription className="text-xs text-gray-500 font-medium">
                Record a student service transaction and calculate counselor commissions for {activeEntity} branch.
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 pt-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <Select value={service} onValueChange={(val) => handleServiceChange(val as CommissionService)}>
                <SelectTrigger id="service" className="h-10 border-gray-200 text-xs font-semibold bg-white">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <Select value={counselor} onValueChange={(val) => handleCounselorChange(val || "Humaira Amin")}>
                <SelectTrigger id="counselor" className="h-10 border-gray-200 text-xs font-semibold bg-white">
          ...
          {/* 4. Fully Received Switch Button */}
          <div className="flex items-center justify-between p-3 rounded-xl border border-gray-200/80 bg-slate-50/70">
            <div className="space-y-0.5">
              <Label htmlFor="fullReceived" className="text-xs font-bold text-gray-900 cursor-pointer">
                Full Received
              </Label>
          ...
          {/* 5. C.C (Counselor Commission) & B.M (Locked / Logical Auto Calculation) */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="cc" className="text-xs font-semibold text-gray-700 flex items-center justify-between">
            ...
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="bm" className="text-xs font-semibold text-gray-700 flex items-center justify-between">
          ...
          {/* 6. Optional Slip / Receipt Upload */}
          <div className="flex flex-col gap-1.5">
            ...
            {slipPreview ? (
              <div className="relative flex items-center justify-between p-3 border border-emerald-200 bg-emerald-50/50 rounded-xl">
                <div className="flex items-center gap-2 text-xs font-semibold text-emerald-900 truncate">
                  ...
                  <span className="truncate">{slipFile?.name || "Receipt uploaded"}</span>
                </div>
                <Button
                  type="button"
                  variant="ghost"
              ...
              >
                ...
                <p className="text-[10px] text-gray-400">PNG, JPG, PDF up to 5MB (leave empty if slip is missing)</p>
                <input
                  ref={fileInputRef}
                  type="file"
